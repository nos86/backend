...
Awesome backend created by nos86
...
```bash
git clone https://github.com/nos86/backend backend
cd backend
make install
```
...
```bash
backend create-db   # run once
backend populate-db  # run once (optional)
backend add-user -u admin -p 1234  # ads a user
backend run
```
