...
original_author="nos86"
original_name="backend"
original_urlname="backend"
original_description="Awesome backend created by nos86" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/backend "${name}"
...
then
    ...
    echo "then run 'backend' or 'python -m backend'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'backend' or 'python -m backend' or 'poetry run backend'"
fi
